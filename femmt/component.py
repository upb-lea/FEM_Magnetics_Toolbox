...
class MagneticComponent:
    ...
    def __init__(self, simulation_type: SimulationType = SimulationType.FreqDomain, component_type: ComponentType = ComponentType.Inductor,
                 working_directory: str = None, clean_previous_results: bool = True, verbosity: Verbosity = 2, is_gui: bool = False,
    ...
    def get_single_complex_permeability(self):
        ...
        if self.core.permeability_type == PermeabilityType.FromData:
            ...
            complex_permeability = mu_0 * mdb.MaterialDatabase(
                self.verbosity == Verbosity.Silent).get_material_attribute(material_name=self.core.material, attribute="initial_permeability")
        ...
        if self.frequency != 0:
            if self.core.permittivity["datasource"] == "measurements" or self.core.permittivity["datasource"] == "datasheet":
                ...
                ff.check_mqs_condition(radius=self.core.core_inner_diameter / 2, frequency=self.frequency,
                                       complex_permeability=self.get_single_complex_permeability(), complex_permittivity=complex_permittivity,
                                       conductivity=self.core.sigma, relative_margin_to_first_resonance=0.5, silent=self.silent)

            else:
                ff.check_mqs_condition(radius=self.core.core_inner_diameter / 2, frequency=self.frequency,
                                       complex_permeability=self.get_single_complex_permeability(), complex_permittivity=0,
                                       conductivity=self.core.sigma, relative_margin_to_first_resonance=0.5, silent=self.silent)
    ...
    def excitation_time_domain(self, current_list: List[List[float]], time_list: List[float], number_of_periods: int, ex_type: str = 'current',
        ...
        self.flag_excitation_type = ex_type  # 'current', 'current_density', 'voltage'
        if self.core.permeability["datasource"] != MaterialDataSource.Custom:
            self.core.update_core_material_pro_file(self.frequency, self.file_data.electro_magnetic_folder_path,
                                                    plot_interpolation)  # frequency update to core class
        if self.core.permittivity["datasource"] != MaterialDataSource.Custom:
    ...
    def component_study(self, time_current_vectors: List[List[List[float]]], fft_filter_value_factor: float = 0.01):
        ...
        :param fft_filter_value_factor: Factor to filter frequencies from the fft. E.g. 0.01 [default] removes all amplitudes
                                        below 1 % of the maximum amplitude from the result-frequency list
    ...
    def stacked_core_center_tapped_pre_study(self, time_current_vectors: List[List[List[float]]], plot_waveforms: bool = False,
        ...
        def split_time_current_vectors_center_tapped(time_current_vectors: List[List[List[float]]]):
            ...
            center_tapped_time_current_vectors = [time_current_vectors[0], [time_current_vectors[1][0], positive_secondary_current],
        ...
        hyst_frequency, hyst_loss_amplitudes, hyst_loss_phases_deg = split_hysteresis_loss_excitation_center_tapped(hyst_frequency, hyst_loss_amplitudes,
                                                                                                                    hyst_loss_phases_deg)
    ...
    def calculate_core_reluctance(self):
        ...
        if self.core.core_type == CoreType.Single:
            ...
            window_reluctance = (fr.r_core_top_bot_radiant(self.core.core_inner_diameter, length_window, self.core.mu_r_abs, self.core.core_inner_diameter/4)*2)
        ...
        # Stacked core
            ...
            # winding window
            length_window = self.core.window_w
            window_reluctance = (fr.r_core_top_bot_radiant(self.core.core_inner_diameter, length_window, self.core.mu_r_abs, self.core.core_inner_diameter/4)*3)
    ...
    def get_inductances(self, I0: float, op_frequency: float = 0, skin_mesh_factor: float = 1, visualize_last_fem_simulation: bool = False,
                        silent: bool = False, compare_with_inductance_from_reluctance: bool = False):
        """
        ...
        * 3 winding transformer
        ...
        if len(self.windings) == 3:
            ...
            inductances = ThreeWindingTransformerInductance(M_12=self.M_12, M_13=self.M_13, M_23=self.M_23,
                                                            L_s1=self.L_s1, L_s2=self.L_s2, L_s3=self.L_s3, L_h=self.L_h,
    ...
    def calc_hystersis_losses_based_on_mesh_results(self, measurement_setup: str = "MagNet", b_wave: np.array = None,
        ...
        if Standard_SE:
            ...
            hyst_dict["Standard_SE"] = np.dot(hyst_losses, volume)
    ...
    def calculate_and_write_freq_domain_log(self, number_frequency_simulations: int = 1, current_amplitude_list: List = None, phase_deg_list: List = None,
                                            frequencies: List = None, core_hyst_losses: List[float] = None, inductance_dict: Optional[List] = None):
        """
        ...
        :param core_hyst_losses: Optional core hysteresis losses value from another simulation. If a value is given, the external value
                                 is used in the result-log. Otherwise, the hysteresis losses of the fundamental frequency is used
            ...
            for single_simulation in range(0, number_frequency_simulations):
                if single_simulation == fundamental_index:
                    for core_part in range(len(self.mesh.plane_surface_core)):
                        ...
                        log_dict["single_sweeps"][single_simulation]["core_parts"][f"core_part_{core_part + 1}"][f"total_core_part_{core_part + 1}"] = \
                            core_hyst_losses[core_part] + log_dict["single_sweeps"][single_simulation]["core_parts"][f"core_part_{core_part +1}"]["eddy_losses"]
    ...
    def log_coordinates_description(self) -> None:
        ...
        for index, winding in enumerate(self.windings):
            ...
            # Rectangular conductors
                ...
                coordinates_dict[f"p_cond_center_{index + 1}"] = [self.two_d_axi.p_conductor[index][i].tolist() for i in
                                                                  range(4, len(self.two_d_axi.p_conductor[index]), 5)]
    ...
    def femm_reference(self, freq: float, current: float, sign: bool = None, non_visualize: int = 0, mesh_size: float = 0.0, mesh_size_conductor: float = 0.0):
        ...
        if self.core.permeability_type == PermeabilityType.FixedLossAngle:
            femm.mi_addmaterial('Ferrite', self.core.mu_r_abs, self.core.mu_r_abs, 0, 0, self.core.sigma / 1e6, 0, 0, 1, 0,
                                self.core.phi_mu_deg, self.core.phi_mu_deg)
            ...
            if self.windings[num].conductor_type in [ConductorType.RoundLitz, ConductorType.RoundSolid]:
                for i in range(0, int(self.two_d_axi.p_conductor[num].shape[0] / 5)):
                    ...
                    femm.mi_drawarc(self.two_d_axi.p_conductor[num][5 * i + 1][0], self.two_d_axi.p_conductor[num][5 * i + 1][1],
                                    self.two_d_axi.p_conductor[num][5 * i + 3][0], self.two_d_axi.p_conductor[num][5 * i + 3][1], 180, 2.5)
                    femm.mi_addarc(self.two_d_axi.p_conductor[num][5 * i + 3][0], self.two_d_axi.p_conductor[num][5 * i + 3][1],
                                   self.two_d_axi.p_conductor[num][5 * i + 1][0], self.two_d_axi.p_conductor[num][5 * i + 1][1], 180, 2.5)
                    femm.mi_addblocklabel(self.two_d_axi.p_conductor[num][5 * i][0], self.two_d_axi.p_conductor[num][5 * i][1])
        ...
        for num in range(len(self.windings)):
            for i in range(0, int(self.two_d_axi.p_conductor[num].shape[0] / 5)):
                ...
                femm.hi_addarc(self.two_d_axi.p_conductor[num][5 * i + 3][0], self.two_d_axi.p_conductor[num][5 * i + 3][1],
                               self.two_d_axi.p_conductor[num][5 * i + 1][0], self.two_d_axi.p_conductor[num][5 * i + 1][1], 180, 2.5)
                femm.hi_addblocklabel(self.two_d_axi.p_conductor[num][5 * i][0], self.two_d_axi.p_conductor[num][5 * i][1])
    ...
    def decode_settings_from_log(log_file_path: str, working_directory: str = None, verbosity: Verbosity = Verbosity.Silent):
        ...
        if settings is not None:
            ...
            geo = MagneticComponent(component_type=ComponentType[settings["component_type"]], working_directory=cwd, verbosity=verbosity)
            ...
            for winding_window in winding_windows:
                ...
                for vww in virtual_winding_windows:
                    ...
                    if winding_type == WindingType.Single:
                        ...
                        new_vww.set_interleaved_winding(conductors[0], turns[0], conductors[1], turns[1],InterleavedWindingScheme[vww["winding_scheme"]],
