...
if component == "transformer":
    ...
    geo.air_gaps.update(method="center", n_air_gaps=1, air_gap_h=[0.0005], air_gap_position=[0])

    geo.update_conductors(n_turns=[[8], [22]], conductor_type=["litz", "litz"],
                          litz_para_type=['implicit_litz_radius', 'implicit_litz_radius'],
                          ff=[0.5, 0.5], strands_numbers=[300, 150], strand_radii=[70e-6, 70e-6],
                          winding=["interleaved"], scheme=["horizontal"],
                          core_cond_isolation=[0.0005, 0.0005], cond_cond_isolation=[0.0002, 0.0002, 0.0005])

    # Perform a single simulation
    geo.single_simulation(freq=250000, current=[8.0, 8 * 8 / 22], phi_deg=[0, 177.5])
