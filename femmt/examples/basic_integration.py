...
import numpy as np
import os
from femmt.examples.femmt_reluctance import MagneticCircuit
...
no_of_turns = list(np.linspace(5, 14, 10))
...
FEM_data_matrix = mc1.data_matrix[np.where((mc1.data_matrix[:, 9] > 0.9 * goal_inductance) *
                                           (mc1.data_matrix[:, 9] < 1.1 * goal_inductance))]
...
P_hyst_center = - 0.5 * (2 * np.pi * freq) * mu_imag * (max_flux / FEM_data_matrix[:, 13] / fmt.mu0 * FEM_data_matrix[:, 3]) ** 2
P_hyst_density_center = P_hyst_center * volume_center

P_hyst_outer = - 0.5 * (2 * np.pi * freq) * mu_imag * (max_flux / FEM_data_matrix[:, 14] / fmt.mu0 * FEM_data_matrix[:, 3]) ** 2
P_hyst_density_outer = P_hyst_outer * volume_outer

P_hyst_density_middle = - 0.5 * (2 * np.pi * freq) * mu_imag * ((max_flux / fmt.mu0 * FEM_data_matrix[:, 3]) ** 2) * (1/(2 * np.pi * FEM_data_matrix[:, 10])) * np.log((FEM_data_matrix[:, 11] * 2) / FEM_data_matrix[:, 0])

P_hyst_density_total = P_hyst_density_center + P_hyst_density_middle + P_hyst_density_outer
FEM_data_matrix = np.hstack((FEM_data_matrix, np.reshape(P_hyst_density_total, (len(P_hyst_density_total), 1)))) # position: 10
print(FEM_data_matrix)
qwerty = 1
