...
pathD = f"C:/Users/tillp/OneDrive/Documents/GitHub/FEM_Magnetics_Toolbox/femmt/MA/final/local_corrected_extended_1/Result_FEM_parameters_200000.npy"
result = np.load(pathD, allow_pickle=True)

# Select a single dictionary
parameters = dict(result[1])
print(parameters)
print(type(parameters))

# parameters["N"] = np.array([[2, 2], [1, 0]])
# parameters["R_top"] = 100e-6
#
...
FEM_results = geo.excitation_sweep(frequencies=[frequencies[0]],
                                   currents=[current_pairs_nom[0]],
                                   phi=[phase_pairs_nom[0]],
                                   show_last=True,
                                   return_results=True)
