...
import femmt.enumerations as enums
...
def plot_3d(x_value: list, y_value: list, z_value: list, x_label: str, y_label: str, z_label: str,
    ...
    def annotate_plot(array_of_points, index):
        ...
        annotate_plot.label = plt.annotate(f'case{index}\nVolume:{x_value_str[index]}\nLoss:{y_value_str[index]}\nCost:{z_value_str[index]}',
                                           xy=(x2, y2), xytext=(-20, 20), bbox=dict(boxstyle='round,pad=0.5', fc='yellow', alpha=0.5), ha='right', va='bottom',
...
class AutomatedDesign:
    ...
    def __init__(self, working_directory: str,
                 ...
                 current_shape: str,  # new variable for project ("triangle" or "sine")
                 offset: bool,  # new variable for project (True or False)
                 peak_current: float,
                 ripple_current: float,  # new variable for project (value of ripple current)
                 frequency: float,
                 duty_cycle: float,  # new variable for project (only relevant for current_shape=="triangle")
                 target_inductance: float,
                 target_inductance_percent_tolerance: int,
                 winding_scheme: str,
                 foil_scheme: str,  # new variable for project ("Vertical" or "Horizontal")
                 ...
                 foil_conductor_thickness: list,  # new variable for project (thickness of foil)
        ...
        self.foil_conductor_thickness = foil_conductor_thickness
    ...
    def set_up_folder_structure(self, working_directory):
        ...
        if working_directory is None:
            ...
            working_directory = os.path.join(os.path.dirname(caller_filename), "inductor_optimization")
        ...
        self.inductor_reluctance_model_results_directory = os.path.join(self.optimization_working_directory, "reluctance_model_results")
        self.inductor_fem_simulations_results_directory = os.path.join(self.optimization_working_directory, "fem_simulation_results")
        self.inductor_optimization_input_parameters_file = os.path.join(self.optimization_working_directory, "optimization_input_parameters.json")
    ...
    def input_pre_process(self):
        ...
        mu_r_abs = [material_db.get_material_attribute(material_name=material_name, attribute="initial_permeability") for material_name in self.core_material]
    ...
    def filter_reluctance_winding_window(self, data_matrix):
        ...
        final_data_matrix3 = np.zeros((len(data_matrix), len(data_matrix[0]) + 6))

        # Adds litz and solid core details to data_matrix with all combinations possible and prepare data matrix for FEM simulation
        # LITZE
        for i in range(len(self.litz_conductor_r)):
            ...
            temp_var7 = np.full((len(data_matrix), 1), np.nan)
            ...
            temp_data_matrix = self.add_column_to_data_matrix(temp_data_matrix, temp_var7, 'foil_conductor_thickness')
        ...
        # FOIL CONDUCTOR
        for j in range(len(self.foil_conductor_thickness)):
            temp_var1 = np.full((len(data_matrix), 1), np.nan)
            temp_var2 = np.full((len(data_matrix), 1), np.nan)
            temp_var3 = np.full((len(data_matrix), 1), np.nan)
            temp_var4 = np.full((len(data_matrix), 1), np.nan)
            temp_var5 = np.full((len(data_matrix), 1), np.nan)
            temp_var6 = np.full((len(data_matrix), 1), np.nan)
            temp_var7 = np.full((len(data_matrix), 1), self.foil_conductor_thickness[j])
            ...
            temp_data_matrix = self.add_column_to_data_matrix(temp_data_matrix, temp_var7, 'foil_conductor_thickness')  # 26

            if j == 0:
                final_data_matrix3 = temp_data_matrix
            else:
                final_data_matrix3 = np.concatenate((final_data_matrix3, temp_data_matrix), axis=0)

        # Handles all the different cases of litz or/and solid conductor inputs  # TODO ADD OTHER POSSIBLE CASES
        # ONLY LITZ CONDUCTOR
        if not len(self.solid_conductor_r) and len(self.litz_conductor_r) and not len(self.foil_conductor_thickness):
            final_data_matrix = final_data_matrix1
        # ONLY SOLID CONDUCTOR
        if not len(self.litz_conductor_r) and len(self.solid_conductor_r) and not len(self.foil_conductor_thickness):
            final_data_matrix = final_data_matrix2
        # ONLY FOIL CONDUCTOR
        elif not len(self.solid_conductor_r) and not len(self.litz_conductor_r) and len(self.foil_conductor_thickness):
            final_data_matrix = final_data_matrix3
        # ONLY LITZ AND FOIL CONDUCTOR
        elif not len(self.solid_conductor_r) and len(self.litz_conductor_r) and len(self.foil_conductor_thickness):
            final_data_matrix = np.concatenate((final_data_matrix1, final_data_matrix3), axis=0)
        # ONLY SOLID AND LITZ CONDUCTOR
        elif len(self.solid_conductor_r) and len(self.litz_conductor_r) and not len(self.foil_conductor_thickness):
            final_data_matrix = np.concatenate((final_data_matrix1, final_data_matrix2), axis=0)
        # ONLY SOLID AND FOIL CONDUCTOR
        elif len(self.solid_conductor_r) and not len(self.litz_conductor_r) and len(self.foil_conductor_thickness):
            final_data_matrix = np.concatenate((final_data_matrix2, final_data_matrix3), axis=0)
        # ALL THREE CONDUCTOR TYPES
        elif len(self.litz_conductor_r) and len(self.solid_conductor_r) and len(self.foil_conductor_thickness):
            final_data_matrix = np.concatenate((final_data_matrix1, final_data_matrix2, final_data_matrix3), axis=0)
        else:
        ...
        # TODO IMPLEMENT CHECK OF WINDING WINDOW WITH FOIL INDUCTOR
        data_matrix = final_data_matrix[
    ...
    def filter_reluctance_losses(self, data_matrix):
        ...
        material_data_list = [material_db.permeability_data_to_pro_file(temperature=self.temperature, datasource=mdb.MaterialDataSource.ManufacturerDatasheet,
                                                                        material_name=material_name, datatype="complex_permeability", frequency=self.frequency)
                              for material_name in self.core_material]
        ...
        if self.offset:
            # flux_max = L * i_max / N
            total_flux_max = (data_matrix[:, self.param["inductance"]] * (self.ripple_current/2)) / data_matrix[:, self.param["no_of_turns"]]
            b_max_center = total_flux_max / data_matrix[:, self.param["center_leg_area"]]
            b_max_outer = total_flux_max / data_matrix[:, self.param["outer_leg_area"]]

            p_hyst_center = 0.5 * (2*np.pi*self.frequency) * fmt.mu_0 * mu_r_imag * ((b_max_center/(fmt.mu_0 * data_matrix[:, self.param["mu_r_abs"]])) ** 2)

            p_hyst_outer = 0.5 * (2*np.pi*self.frequency) * fmt.mu_0 * mu_r_imag * ((b_max_outer/(fmt.mu_0 * data_matrix[:, self.param["mu_r_abs"]])) ** 2)

            p_hyst_density_center = p_hyst_center * volume_center
            p_hyst_density_middle = 0.5 * (2 * np.pi * self.frequency) * mu_r_imag * fmt.mu_0 * \
                                    ((total_flux_max / (fmt.mu_0 * data_matrix[:, self.param["mu_r_abs"]])) ** 2) * \
            ...
            p_hyst_center = 0.5 * (2*np.pi*self.frequency) * fmt.mu_0 * mu_r_imag * ((data_matrix[:, self.param["b_max_center"]] /
            ...
            p_hyst_density_middle = 0.5 * (2*np.pi*self.frequency) * mu_r_imag * fmt.mu_0 * \
                                    ...
                                    np.log((data_matrix[:, self.param["r_inner"]] * 2) / data_matrix[:, self.param["core_inner_diameter"]])
            p_hyst_density_outer = p_hyst_outer * volume_outer
        ...
        dc_resistance_wire = []
        for index, value in enumerate(data_matrix):
            if np.isnan(data_matrix[index, self.param["foil_conductor_thickness"]]):
                dc_resistance_wire.append((data_matrix[index, self.param["no_of_turns"]]*2*np.pi*(data_matrix[index, self.param["core_inner_diameter"]]/2 + \
                                                                                                  data_matrix[index, self.param["conductor_radius"]])) / \
                                          (self.copper_conductivity * (np.pi * (data_matrix[index, self.param["conductor_radius"]]**2))))
            else:
                dc_resistance_wire.append((data_matrix[index, self.param["no_of_turns"]] * ((data_matrix[index, self.param["core_inner_diameter"]] +\
                                                                                             data_matrix[index, self.param["window_w"]])/2)) / \
                                          (self.copper_conductivity * (data_matrix[index, self.param["window_w"]] * \
                                                                       data_matrix[index, self.param["foil_conductor_thickness"]])))
                # print(data_matrix[index, self.param["window_w"]])

        dc_resistance_wire = np.array(dc_resistance_wire)

        # I^2 * R loss. Calculation from PEAK current, so division by 2 is needed
        if self.offset:
            dc_wire_loss = ((self.peak_current - self.ripple_current/2)**2) * dc_resistance_wire  # Assuming current waveform with offset
        else:
        ...
        data_matrix = self.add_column_to_data_matrix(data_matrix, total_hyst_loss, 'total_hyst_loss')                       # 27
        data_matrix = self.add_column_to_data_matrix(data_matrix, dc_wire_loss, 'dc_wire_loss')                             # 28
        data_matrix = self.add_column_to_data_matrix(data_matrix, total_hyst_dc_loss, 'total_loss')                         # 29
        data_matrix = self.add_column_to_data_matrix(data_matrix, normalized_total_loss, 'normalized_total_loss')           # 30
        data_matrix = self.add_column_to_data_matrix(data_matrix, core_2daxi_volume, 'total_volume')                        # 31
        data_matrix = self.add_column_to_data_matrix(data_matrix, normalized_core_2daxi_volume, 'normalized_total_volume')  # 32
    ...
    def fem_simulation(self, count=None):
        ...
        if self.current_shape == "sine":
            ...
            for count in cases:
                ...
                if int(self.data_matrix_fem[count, self.param["n_air_gaps"]]) == 1:
                    air_gaps.add_air_gap(fmt.AirGapLegPosition.CenterLeg, self.data_matrix_fem[count, self.param["air_gap_h"]],
                                         self.data_matrix_fem[count, self.param["air_gap_position"]])
                else:
                    if int(self.data_matrix_fem[count, self.param["mult_air_gap_type"]]) == 1:
                        position_list = list(np.linspace(0, 100, int(self.data_matrix_fem[count, self.param["n_air_gaps"]])))
                        for position in position_list:
                            air_gaps.add_air_gap(fmt.AirGapLegPosition.CenterLeg, self.data_matrix_fem[count, self.param["air_gap_h"]], position)

                    elif int(self.data_matrix_fem[count, self.param["mult_air_gap_type"]]) == 2:
                        position_list = list(np.linspace(0, 100, int(self.data_matrix_fem[count, self.param["n_air_gaps"]]) + 2))
                ...
                insulation.add_core_insulations(self.top_core_insulation, self.bot_core_insulation, self.left_core_insulation, self.right_core_insulation)
                ...
                winding = fmt.Conductor(0, fmt.Conductivity.Copper, winding_material_temperature=self.temperature)

                if np.isnan(self.data_matrix_fem[count, self.param["solid_conductor_r"]]) and\
                        np.isnan(self.data_matrix_fem[count, self.param["foil_conductor_thickness"]]):
                    print("LITZ-CONDUCTOR")
                ...
                elif np.isnan(self.data_matrix_fem[count, self.param["solid_conductor_r"]]) and\
                        np.isnan(self.data_matrix_fem[count, self.param["litz_conductor_r"]]):  # TODO IMPLEMENT FOIL CONDUCTOR
                    # winding.set_rectangular_conductor(thickness=self.foil_conductor_thickness)
                    print("FOIL-CONDUCTOR")
                    winding.set_rectangular_conductor(thickness=None if np.isnan(self.data_matrix_fem[count, self.param["foil_conductor_thickness"]]) else \
                                                      self.data_matrix_fem[count, self.param["foil_conductor_thickness"]])
                    if self.foil_scheme == "Horizontal":
                        vww.set_winding(winding, int(self.data_matrix_fem[count, self.param["no_of_turns"]]), fmt.WindingScheme.FoilHorizontal,
                                        wrap_para_type=fmt.WrapParaType.FixedThickness)
                    elif self.foil_scheme == "Vertical":
                        vww.set_winding(winding, int(self.data_matrix_fem[count, self.param["no_of_turns"]]), fmt.WindingScheme.FoilVertical,
                                        wrap_para_type=fmt.WrapParaType.FixedThickness)

                else:
                    print("SOLID-CONDUCTOR")
                ...
                try:
                    ...
                    destination_json_file = os.path.join(self.inductor_fem_simulations_results_directory, f'case_{count+1}.json')
        ...
        if self.current_shape == "triangle":
            ...
            time_array = np.linspace(0, 1/self.frequency, 101)

            points = np.linspace(0, 100, 101)
            i_on = [self.peak_current - self.ripple_current * (1 - x / (self.duty_cycle * 100)) for x in points[0:int(self.duty_cycle * 100 + 1)]]
            i_off = [self.peak_current - self.ripple_current * ((x - self.duty_cycle * 100) / ((1 - self.duty_cycle) * 100))
                     for x in points[int(self.duty_cycle * 100) + 1:]]
            current_array = np.array(i_on + i_off)

            fft_filter = 0.01

            # loop until 6 frequency point -> DC(0HZ) + f0,fundamentale frequency and 4 largest harmonic frequencys
            fft_frequency = []
            while len(fft_frequency) <= 5:
                fft = femmt.fft(period_vector_t_i=[time_array, current_array], plot='no', mode='time', f0=self.frequency, title='ffT input current',
                                filter_value_factor=fft_filter)
                fft_filter = fft_filter/2

                fft_frequency = fft[0]
                fft_current_ampl = fft[1]
                fft_current_phase = fft[2]

            for index, value in enumerate(fft_frequency):
                if index == 0:
                    print("DC-part: ", value, "Hz")
                    print("DC-part: ", fft_current_ampl[index], "A")
                    print("DC-part: ", fft_current_phase[index], "°")
                elif index == 1:
                    print("fundamental frequency: ", value, "Hz")
                    print("fundamental frequency: ", fft_current_ampl[index], "A")
                    print("fundamental frequency: ", fft_current_phase[index], "°")
                else:
                    print(int(value/fft_frequency[1]), ". harmonic frequency: ", value, "Hz")
                    print(int(value/fft_frequency[1]), ". harmonic frequency: ", fft_current_ampl[index], "A")
                    print(int(value/fft_frequency[1]), ". harmonic frequency: ", fft_current_phase[index], "°")

            # correct format for excitation_sweep
            fft_current_ampl = [[x] for x in fft_current_ampl]
            fft_current_phase = [[x] for x in fft_current_phase]
            # append values for hysteresis losses (fundamental frequency + current_ripple)
            fft_frequency = np.append(fft_frequency, self.frequency)
            fft_current_ampl.append([self.ripple_current/2])
            fft_current_phase.append(fft_current_phase[1])
            ...
            for count in cases:
                ...
                core = fmt.Core(core_type=fmt.CoreType.Single, core_dimensions=core_dimensions, temperature=self.temperature, frequency=self.frequency,
                                material=self.core_material_dict[self.data_matrix_fem[count, self.param["mu_r_abs"]]],
                ...
                if np.isnan(self.data_matrix_fem[count, self.param["solid_conductor_r"]]) and\
                    ...
                    vww.set_winding(winding, int(self.data_matrix_fem[count, self.param["no_of_turns"]]), None)
                        ...
                        np.isnan(self.data_matrix_fem[count, self.param["litz_conductor_r"]]):
                    print("FOIL-CONDUCTOR")
                    winding.set_rectangular_conductor(thickness=None if np.isnan(self.data_matrix_fem[count, self.param["foil_conductor_thickness"]]) else \
                                                      self.data_matrix_fem[count, self.param["foil_conductor_thickness"]])
                    if self.foil_scheme == "Horizontal":
                ...
                try:
                    # print("window_w: ", self.data_matrix_fem[count, self.param["window_w"]])
                    # print("window_h: ", self.data_matrix_fem[count, self.param["window_h"]])
                    # print("core_inner_diameter: ", self.data_matrix_fem[count, self.param["core_inner_diameter"]])
                    # print("solid_conductor_r: ", self.data_matrix_fem[count, self.param["solid_conductor_r"]])
                    # print("foil_conductor_thickness: ", self.data_matrix_fem[count, self.param["foil_conductor_thickness"]])
                    print("foil_conductor_thickness: ", self.data_matrix_fem[count, self.param["b_max_center"]])
                    ...
                    geo.excitation_sweep(frequency_list=fft_frequency, current_list_list=fft_current_ampl, phi_deg_list_list=fft_current_phase,
                                         show_last_fem_simulation=False, excitation_meshing_type=enums.ExcitationMeshingType.MeshOnlyHighestFrequency)
            ...
            print("No known current shape passed. No simultaions started!")
    ...
    def save_automated_design_settings(self):
        ...
        dictionary = {
            ...
            "foil_scheme": self.foil_scheme,
            "peak_current": self.peak_current,
            "ripple_current": self.ripple_current,
            ...
            "foil_conductor_thickness": self.foil_conductor_thickness,
...
if __name__ == '__main__':
    ...
    if task == 'simulate':
        ...
        pq2020 = core_database["PQ 20/20"]  # list(np.linspace(min_core['core_inner_diameter'], max_core['core_inner_diameter'], 3)),
        pq3230 = core_database["PQ 32/30"]  # list(np.linspace(min_core['window_h'], max_core['window_h'], 3)),
        pq4040 = core_database["PQ 40/40"]  # list(np.linspace(min_core['window_w'], max_core['window_w'], 3)),
        ...
        ad = AutomatedDesign(working_directory=working_directory,
                             ...
                             offset=True,
                             target_inductance=6 * 1e-6,
                             frequency=1000000,
                             duty_cycle=0.25,
                             target_inductance_percent_tolerance=10,
                             winding_scheme='Hexagonal',
                             foil_scheme='Horizontal',
                             peak_current=42.4167,
                             ripple_current=1.5,
                             percent_of_flux_density_saturation=80,
                             percent_of_total_loss=40,
                             database_core_names=[],
                             database_litz_names=["4.0x2160x0.071"],
                             solid_conductor_r=[np.sqrt(6 / np.pi) * 1e-3],
                             foil_conductor_thickness=[],  # [0.5*1e-3, 1*1e-3],
                             manual_core_inner_diameter=list(np.linspace(8, 14, 7) * 1e-3),
                             manual_window_h=list(np.linspace(15, 25, 11) * 1e-3),
                             manual_window_w=list(np.linspace(8, 13, 6) * 1e-3),
                             no_of_turns=np.arange(4, 6).tolist(),
                             n_air_gaps=[1, 2, 3],
                             air_gap_height=[0.38 * 1e-3, 0.5 * 1e-3, 0.76 * 1e-3, 0.88 * 1e-3, 1.0 * 1e-3, 1.14 * 1e-3, 1.26 * 1e-3, 1.38 * 1e-3, 1.5 * 1e-3],
                             air_gap_position=[50],
                             core_material=[fmt.Material.N49],
                             ...
                             inner_winding_insulation=0.001,
                             temperature=60.0,
        ...
        working_directory = "F:/schacht/Python_Projects/FEM_Magnetics_Toolbox/femmt/examples/example_results/inductor_optimization"

        # all_files = [f for f in os.listdir(os.path.join(working_directory, "fem_simulation_results"))
        #              if os.path.isfile(os.path.join(working_directory, "fem_simulation_results", f))]
        # print(all_files)
