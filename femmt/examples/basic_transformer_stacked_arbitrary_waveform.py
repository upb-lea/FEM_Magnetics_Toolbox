...
def basic_example_transformer_stacked_arbitrary_waveform(onelab_folder: str = None, show_visual_outputs: bool = True,
                                                         is_test: bool = False):
    ...
    working_directory = os.path.join(example_results_folder, "stacked_transformer_arbitrary_waveform")
    if not os.path.exists(working_directory):
        os.mkdir(working_directory)

    # 1. chose simulation type
    geo = fmt.MagneticComponent(component_type=fmt.ComponentType.IntegratedTransformer,
                                working_directory=working_directory, verbosity=fmt.Verbosity.Silent, is_gui=is_test)
    ...
    # geo.single_simulation(freq=250000, current=[2.0, 4.0], phi_deg=[0, 180], show_fem_simulation_results=show_visual_outputs)
...
if __name__ == "__main__":
    basic_example_transformer_stacked_arbitrary_waveform(show_visual_outputs=False)
