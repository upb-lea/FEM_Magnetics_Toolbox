...
class IntegratedTransformerOptimization:
    ...
    ###################################################################################
    # reluctance model functions
    ###################################################################################
    @staticmethod
    def filter_reluctance_model_list(valid_design_list: List[ItoSingleResultFile], factor_min_dc_losses: float = 1.2) -> \
    ...
    @classmethod
    def integrated_transformer_optimization(cls, config_file: ItoSingleInputConfig):
        case_number = 0
        ...
        sweep_dto = cls.calculate_sweep_tensors(config_file)
        ...
        number_of_simulations = len(sweep_dto.t1_core_material) * len(sweep_dto.t1_window_w) * len(
            sweep_dto.t1_window_h_top) * len(sweep_dto.t1_window_h_bot) * len(sweep_dto.t1_core_inner_diameter) * len(
            sweep_dto.t1_n_p_top) * len(sweep_dto.t1_n_p_bot) * len(sweep_dto.t1_n_s_top) * len(sweep_dto.t1_n_s_bot)
        ...
        for count_core_material, material_name in enumerate(sweep_dto.t1_core_material):
            ...
            """
            mu_r_abs = material_db.get_material_property(material_name=material_name, property="initial_permeability")
                        ...
                        for count_n_s_top, n_s_top in enumerate(sweep_dto.t1_n_s_top):
                            ...
                            for count_n_p_bot, n_p_bot in enumerate(sweep_dto.t1_n_p_bot):
                                ...
                                for count_n_s_bot, n_s_bot in enumerate(sweep_dto.t1_n_s_bot):
                                    ...
                                    t2_reluctance_matrix = cls.t2_calculate_reluctance_matrix(t2_inductance_matrix,
                                                                                              t2_winding_matrix,
                                        ...
                                        if (flux_density_top_max < dimensioning_max_flux_density) and (
                                            ...
                                            if l_top_air_gap > minimum_sort_out_air_gap_length and l_bot_air_gap > minimum_sort_out_air_gap_length and l_middle_air_gap > minimum_sort_out_air_gap_length:
                                                p_hyst_top = fr.hyst_losses_core_half_mu_r_imag(core_inner_diameter,
                                                                                                window_h_top, window_w,
                                                                                                mu_r_abs, flux_top_max,
                                                                                                fundamental_frequency,
                                                ...
                                                p_hyst_middle = fr.power_losses_hysteresis_cylinder_radial_direction_mu_r_imag(
                                                    ...
                                                    core_inner_diameter / 2, core_inner_diameter / 2 + window_w,
                                                    fundamental_frequency,
    ...
    @staticmethod
    def load_reluctance_model_list(filepath: str) -> List[ItoSingleResultFile]:
        """
    ...
    @classmethod
    def load_reluctance_model_result_list(cls, working_directory: str) -> List[ItoSingleResultFile]:
        """
        ...
        return cls.load_reluctance_model_list(integrated_transformer_reluctance_model_results_directory)
    ...
    @classmethod
    def load_reluctance_model_filtered_result_list(cls, working_directory: str) -> List[ItoSingleResultFile]:
        """
        Loads the results of the reluctance model and returns the ItoSingleResultFiles as a list.
    ...
    @classmethod
    def save_reluctance_model_result_list(cls, config_file: ItoSingleInputConfig, result_file_list: List[ItoSingleResultFile]):
        """
        ...
        cls.set_up_folder_structure(config_file.working_directory)
        ...
        cls.save_dto_list(result_file_list, integrated_transformer_reluctance_model_results_directory)



    ###################################################################################
    # FEM simulation functions
    ###################################################################################

    @staticmethod
    def load_fem_simulations(filepath: str) -> List[Dict]:
        """
    ...
    @staticmethod
    def filter_fem_simulations(fem_simulations_dict_list: List[Dict]):
        """"
    ...
    def load_fem_simulation_results(working_directory: str):
        ...
        integrated_transformer_fem_simulations_result_directory = os.path.join(working_directory, "02_fem_simulation_results")
        ff.femmt_print(f"Read results from {integrated_transformer_fem_simulations_result_directory}")

    @staticmethod
    def plot_fem_simulation_result_list(working_directory: str) -> None:
        """
    ...
    def thermal_simulation(self, fem_simulations_dict_list: List[Dict]):
        pass
